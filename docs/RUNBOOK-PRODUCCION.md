...
## Migración de datos (importación masiva)

Desde la UI con permiso `*.create` correspondiente:

| Módulo | Ruta | Formato | Plantilla |
|--------|------|---------|-----------|
| Clientes | `/crm` | CSV | Botón «Plantilla CSV» |
| Proveedores | `/proveedores` | CSV | Botón «Plantilla CSV» |
| Inventario | `/inventario` | CSV o Excel | «Plantilla CSV» / «Plantilla Excel» |

**Inventario CSV** — columnas: `sku` (o `codigo`), `nombre`, `stock`, `stockMinimo`, `precio` (opcional). Idempotente por SKU (actualiza si ya existe).

API: `POST /api/inventario/import` · plantilla: `GET /api/inventario/plantilla?formato=csv`

Recomendado: importar en lotes &lt; 500 filas y revisar el resumen (creados / actualizados / errores).
