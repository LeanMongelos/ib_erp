...
| Cron HTTP | — | `POST /api/cron/cobranzas-vencimientos`, `POST /api/cron/alquiler-cuotas`, `POST /api/cron/ots-vencidas`, `POST /api/cron/presupuestos-vencidos` + `CRON_SECRET` | `app/api/cron/` |
